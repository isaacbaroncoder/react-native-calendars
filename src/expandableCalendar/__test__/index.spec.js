...
describe("ExpandableCalendar", () => {
  ...
  describe("Init", () => {
    ...
    it("should display today's date", () => {
      ...
      expect(getMonthTitle(driver.getCalendarList().props.data[50])).toBe(
        getMonthTitle(today)
      );
      // WeekCalendar
    ...
    describe("arrow press", () => {
      ...
      it.each([[Direction.LEFT], [Direction.RIGHT]])(
        ...
        (direction) => {
          ...
          expect(onDateChanged).toHaveBeenCalledWith(
            toMarkingFormat(expectedDate),
            UpdateSources.PAGE_SCROLL
          );
          expect(onMonthChange).toHaveBeenCalledWith(
            xdateToData(expectedDate),
            UpdateSources.PAGE_SCROLL
          );
        }
      );
      it(`should call onDateChanged and onMonthChanged for first day in initial month when changing to initial month`, () => {
        ...
        expect(onMonthChange).toHaveBeenNthCalledWith(
          2,
          xdateToData(expectedDate),
          UpdateSources.PAGE_SCROLL
        );
      ...
      it(`should navigate 6 months ahead and back successfully`, () => {
        ...
        expect(onMonthChange).toHaveBeenNthCalledWith(
          6,
          xdateToData(expectedFutureDate),
          UpdateSources.PAGE_SCROLL
        );
  ...
  describe("WeekCalendar updates", () => {
    ...
    describe("arrow press", () => {
      ...
      it.each([
        ...
        `should call onDateChanged to %s week first day when pressing %s arrow`,
        (direction) => {
          const currentDay = today.getDay();
          const expectedDate = today
            .clone()
            .addDays(
              direction === Direction.LEFT ? -(currentDay + 7) : 7 - currentDay
            );
          driver.pressOnHeaderArrow({left: direction === Direction.LEFT});
          expect(onDateChanged).toHaveBeenCalledWith(
            toMarkingFormat(expectedDate),
      ...
      it("should fetch next weeks when in last week of the list", () => {
        times(NUMBER_OF_PAGES + 1, () =>
          driver.pressOnHeaderArrow({left: false})
        );
        const currentDay = today.getDay();
        const expectedDate = today
          .clone()
          .addDays(7 * (NUMBER_OF_PAGES + 1) - currentDay);
      ...
      it("should call onMonthChange when new week first day is in a different month", () => {
        ...
        const expectedDate = today
          .clone()
          .setDate(today.getDate() + 7 * diff - today.getDay());
  ...
  describe.skip("today button", () => {
    ...
    it.each([
      ...
      "should %sappear when the initial date %s today",
      (_message1, _message2, isToday) => {
    ...
    it(`should call onDateChanged with today's date when today button pressed`, () => {
      ...
      expect(onDateChanged).toHaveBeenCalledWith(
        toMarkingFormat(today),
        UpdateSources.TODAY_PRESS
      );
    });
