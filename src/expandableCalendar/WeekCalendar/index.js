...
const WeekCalendar = (props) => {
  const {calendarWidth, hideDayNames, current, theme, testID, markedDates} =
    props;
  ...
  const {date, numberOfDays, updateSource, setDate, timelineLeftInset} =
    context;
  ...
  useDidUpdate(() => {
    if (updateSource !== UpdateSources.WEEK_SCROLL) {
      ...
      if (pageIndex !== currentIndex.current) {
        const adjustedIndexFrScroll = constants.isAndroidRTL
          ? NUM_OF_ITEMS - 1 - pageIndex
          : pageIndex;
        if (pageIndex >= 0) {
  ...
  const renderItem = useCallback(
    ({item}) => {
      const currentContext = sameWeek(date, item, firstDay)
        ? context
        : undefined;
    ...
    ({viewableItems}) => {
      ...
      if (newDate !== visibleWeek.current) {
        if (constants.isAndroidRTL) {
          ...
          const newDateOffset =
            -1 * (NUMBER_OF_PAGES - currItems.indexOf(newDate));
  ...
  return (
    <View testID={testID} style={weekCalendarStyle}>
      {!hideDayNames && (
        <View style={containerStyle}>{renderWeekDaysNames}</View>
      )}
...
function getDate(date, firstDay, weekIndex, numberOfDays) {
  ...
  const newDate =
    numberOfDays && numberOfDays > 1
      ? d.addDays(weekIndex * numberOfDays)
      : d.addWeeks(weekIndex);
