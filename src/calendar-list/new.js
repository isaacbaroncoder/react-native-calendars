...
const CalendarList = (props) => {
  ...
  const getMonthIndex = useCallback(
    (month) => {
      ...
      return items.findIndex((item) =>
        item.includes(month.toString("yyyy-MM"))
      );
    },
    [items]
  );
  const scrollToMonth = useCallback(
    (month) => {
      if (month) {
        ...
        if (index !== -1) {
          const shouldAnimate =
            constants.isAndroid && !horizontal ? false : true;
          // @ts-expect-error
          list.current?.scrollToOffset?.(
            index * constants.screenWidth,
            0,
            shouldAnimate
          );
        }
  ...
  const listContainerStyle = useMemo(() => {
    return [
      style.current.flatListContainer,
